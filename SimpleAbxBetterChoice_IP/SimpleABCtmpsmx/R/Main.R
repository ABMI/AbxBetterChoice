...
#'
#' @details
#' This function executes the SimpleABCtmpsmx Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    SimpleABCtmpsmx::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "SimpleABCtmpsmx")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- SimpleABCtmpsmx::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    SimpleABCtmpsmx::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
