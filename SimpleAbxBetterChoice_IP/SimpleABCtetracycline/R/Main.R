...
#
# This file is part of SimpleABCtetracycline
#
...
#'
#' @details
#' This function executes the SimpleABCtetracycline Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    SimpleABCtetracycline::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "SimpleABCtetracycline")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- SimpleABCtetracycline::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    SimpleABCtetracycline::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
