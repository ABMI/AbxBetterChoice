...
#
# This file is part of SimpleABCgentamicin
#
...
#'
#' @details
#' This function executes the SimpleABCgentamicin Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    SimpleABCgentamicin::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "SimpleABCgentamicin")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- SimpleABCgentamicin::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    SimpleABCgentamicin::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
