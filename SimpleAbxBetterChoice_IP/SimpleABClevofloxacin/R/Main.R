...
#
# This file is part of SimpleABClevofloxacin
#
...
#'
#' @details
#' This function executes the SimpleABClevofloxacin Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    SimpleABClevofloxacin::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "SimpleABClevofloxacin")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- SimpleABClevofloxacin::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    SimpleABClevofloxacin::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
