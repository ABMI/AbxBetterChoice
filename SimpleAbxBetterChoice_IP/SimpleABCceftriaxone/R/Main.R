...
#
# This file is part of SimpleABCceftriaxone
#
...
#'
#' @details
#' This function executes the SimpleABCceftriaxone Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    SimpleABCceftriaxone::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "SimpleABCceftriaxone")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- SimpleABCceftriaxone::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    SimpleABCceftriaxone::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
