...
#
# This file is part of ABCtetracycline
#
...
#'
#' @details
#' This function executes the ABCtetracycline Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    ABCtetracycline::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "ABCtetracycline")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- ABCtetracycline::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    ABCtetracycline::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
