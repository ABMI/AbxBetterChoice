...
#
# This file is part of ABCgentamicin
#
...
#'
#' @details
#' This function executes the ABCgentamicin Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    ABCgentamicin::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "ABCgentamicin")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- ABCgentamicin::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    ABCgentamicin::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
