...
#
# This file is part of ABClevofloxacin
#
...
#'
#' @details
#' This function executes the ABClevofloxacin Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    ABClevofloxacin::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      pathToCustom <- system.file("settings", paste0(cohortVariableSetting, ".csv"), package = "ABClevofloxacin")
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- ABClevofloxacin::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    ABClevofloxacin::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
