...
#
# This file is part of ABCciprofloxacin
#
...
#'
#' @details
#' This function executes the ABCciprofloxacin Study.
#' 
...
execute <- function(connectionDetails,
  ...
  if (createCohorts) {
    ...
    ABCciprofloxacin::createCohorts(connectionDetails = connectionDetails,
  ...
  if(runAnalyses){
    ...
    if(!is.null(cohortVariableSetting)){
      ...
      for(i in 1:nrow(cohortVarsToCreate)){
        cohortCov[[1+i]] <- ABCciprofloxacin::createCohortCovariateSettings(covariateName = as.character(cohortVarsToCreate$cohortName[i]),
  ...
  if (createShiny) {
    ABCciprofloxacin::populateShinyApp(outputDirectory = file.path(outputFolder, 'ShinyApp'),
